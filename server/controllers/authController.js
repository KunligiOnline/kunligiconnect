  ...
  db.query(queryText, [username])
    .then(data => {
      ...
      return next({
        ...
        message: { err: 'authController.checkUserExists: ERROR: Check server log for details.'}
      });
    })
}
  ...
  db.query(queryText, [username, email, password])
    .then(data => {
      ...
      return next({
        ...
        message: { err: 'authController.signUp: ERROR: Check server log for details.'}
      });
    })
};
  ...
  db.query(queryText, [username])
    .then(data => {
      if (data.rows[0].password === password) {
        ...
        message: { err: 'authController.logIn: ERROR: Check server log for details.' }}));
  }
